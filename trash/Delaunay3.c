...
void usage_main(void)
{
  ...
  //printf("Usage:./Densfield <Gadget_snapshoth_file> <param_file>\n");
  exit(SUCCES);
}
    ...
    for (int i = 0; i < (int)dims[0]; i++) {
      ...
      for (int j = 0; j < Number_Processes; j++) {
        ...
        if((Npos[0] >= domain_info[j].xmin) && (Npos[0] <= domain_info[j].xmax)) {
	        ...
	        if((Npos[1] >= domain_info[j].ymin) && (Npos[1] <= domain_info[j].ymax)) {
       
	          if((Npos[2] >= domain_info[j].zmin) && (Npos[2] <= domain_info[j].zmax)) {
              
              //The box contains particles of type 0 (inside the domain or neighbours to the domain) 
              //and type 1 (just the ones inside the domain)
              Rpart[ID].BoxID = j;
              Rpart[ID].FileID = fileID;
              Rpart[ID].ID = i; //this id the ID of the particle in the file
              Rpart[ID].tag = 0;
              //Rpart[ID].Oid = ID; //this is the ID of the particle in the whole simulation
              Rpart[ID].Volume = 0.0;
