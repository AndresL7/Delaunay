...
int main(int argc, char *argv[])
{
  ...
  struct part dp;
    for (int i = 0; i < (int)dims[0]; i++) {
      ...
      // Asegurarse de no salirse del grid:

      int ix_min = (IDx - 1 < 0) ? 0 : IDx - 1;
      int iy_min = (IDy - 1 < 0) ? 0 : IDy - 1;
      int iz_min = (IDz - 1 < 0) ? 0 : IDz - 1;
      int ix_max = (IDx + 1 >= Np1D) ? Np1D - 1 : IDx + 1;
      int iy_max = (IDy + 1 >= Np1D) ? Np1D - 1 : IDy + 1;
      int iz_max = (IDz + 1 >= Np1D) ? Np1D - 1 : IDz + 1;
