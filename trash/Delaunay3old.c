...
int main(int argc, char *argv[])
{
  ...
  int i,Tini,counter,Np1D=30,j,k,l,ipart;
  ...
  //Let's read all the particles and assign a BoxID and an ID to each one of them corresponding to the domain they belong to
  //We will use the domain_info structure to store the BoxID of each particle 
  //The idea is to have each particle well identified (ID) with the domain it belongs to (BoxID)
  int ID = 0;
  printf("Npart_Total = %d\n", Npart_Total);
  
  Rpart = (struct part *) malloc((size_t) Npart_Total*sizeof(struct part));
  ...
  {  
    ...
    printf("entering to domain assign...");
    for (int i = 0; i < dims[0]; i++) {
      for (int j = 0; j < Number_Processes; j++) {
        if (positions[i][0] >= domain_info[j].xmin && positions[i][0] < domain_info[j].xmax &&
            positions[i][1] >= domain_info[j].ymin && positions[i][1] < domain_info[j].ymax &&
            positions[i][2] >= domain_info[j].zmin && positions[i][2] < domain_info[j].zmax) {
              Rpart[ID].BoxID = j;
              //Rpart[ID].pos[0] = positions[i][0];
              //Rpart[ID].pos[1] = positions[i][1];
              //Rpart[ID].pos[2] = positions[i][2];
              Rpart[ID].Volume = 0.0;
              Rpart[ID].ID = i;
              ID = ID + 1;
              break;
  ...
  for(l=0; l<Number_Processes; l++)
      {
        ...
        int Nparts = GalCos_load_Gadget(infile,overlap,l);
